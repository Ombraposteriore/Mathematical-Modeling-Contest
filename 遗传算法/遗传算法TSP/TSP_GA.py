...
def draw_path(line, CityCoordinates):
    ...
    y.append(y[0])
    plt.subplot(1, 2, 1)
    plt.plot(x, y, 'mo:', color='#4169E1', alpha=0.8, linewidth=0.8)
    plt.xlabel('x')
    plt.ylabel('y')
    # plt.show()
...
if __name__ == '__main__':
    ...
    # 计算城市之间的距离
    col = range(len(CityCoordinates))
    ind = range(len(CityCoordinates))
    dis_matrix = pd.DataFrame(data=0, columns=col, index = ind)
    ...
    while iteration <= generation:
        ...
        #plt.show()
        f = plt.gcf()
        save_name = r'{0}.jpg'.format(iteration)
        f.savefig(save_name)
        f.clear()
